function FVideo($scope, $http, $window, $location) {
    ...
    // Get the Video Count 
    $http.get('/api/videos/count').success(function(vidCount) {
    ...
    if (location.pathname == '/'){
        skipValue =0;
        urlToQuery = '/api/videos/featured_none?limit=' + $scope.videosPerPage + '&skip=0';
    }
    else{

        $scope.currentPageNumber = parseInt(getURLParameter('p'));
        skipValue = ($scope.currentPageNumber - 1) * $scope.videosPerPage;
        urlToQuery = '/api/videos/featured_none?limit=' + $scope.videosPerPage + '&skip=' + skipValue;
    }
    ...
    //Latest Videos
    $http.get('/api/videos/latest').success(function(data4) {
...
function IVideo($scope, $http, $window){
    ...
    $scope.init = function(id){
        $scope.video_id = id;
        qUrl = '/api/videos/single_video/' + $scope.video_id;
...
function SimilarVideos($scope,$http){
    ...
    var urlToQuery = '/api/videos/featured_none?limit=' + $scope.videosPerPage + '&skip=0';
